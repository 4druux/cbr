...
$sql_get_gejala =
    "SELECT gejala.* FROM gejala, tmp_analisa WHERE gejala.kd_gejala=tmp_analisa.kd_gejala AND tmp_analisa.noip=? AND NOT tmp_analisa.kd_gejala IN (SELECT kd_gejala FROM tmp_gejala WHERE noip=?) ORDER BY gejala.kd_gejala LIMIT 1";
...
<div class="panel panel-default">
    ...
    <tbody>
    <?php
    ($query = $koneksi->query("SELECT * FROM gejala")) or
        die("Query Error: " . $koneksi->error);
    while ($row = $query->fetch_assoc()) { ?>
    <tr>
        <td style="text-align:center;width:5%;"><input type="checkbox" name="gejala[]" id="gejala" value="<?php echo htmlspecialchars(
            $row["kd_gejala"],
        ); ?>"></td>
    ...
    <tr>
        ...
        <td><strong>Tidak ada gejala diatas</strong></td>
    </tr>
    </tbody>
    </table>
</div>
